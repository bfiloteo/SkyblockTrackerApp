...
public final class QueryUtilsForSpider {

    private static final String LOG_TAG = com.example.capstoneproject.Slayers.Spider.QueryUtilsForSpider.class.getSimpleName();

    private QueryUtilsForSpider() {
    }

    public static List<Spider> fetchStatData(String requestUrl) {
        // Create URL object
        ...
        List<Spider> spiderList = extractFeatureFromJson(jsonResponse);
    ...
    private static List<Spider> extractFeatureFromJson(String statsJSON) {
        ...
        List<Spider> spiderElement = new ArrayList<>();
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                // the slayer.
                int tierOneSpiderKills = 0;
                if(spiderKills.has("1")) {
                    tierOneSpiderKills = spiderKills.getInt("1");
                }

                int tierTwoSpiderKills = 0;
                if(spiderKills.has("2")) {
                    tierTwoSpiderKills = spiderKills.getInt("2");
                }

                int tierThreeSpiderKills = 0;
                if(spiderKills.has("3")){
                    tierThreeSpiderKills = spiderKills.getInt("3");
                }

                int tierFourSpiderKills = 0;
                if(spiderKills.has("4")) {
                    tierFourSpiderKills = spiderKills.getInt("4");
                }

                int coinsSpentOnSpiders = 0;
                if(coinsSpentOnSlayers.has("spider")) {
                    coinsSpentOnSpiders = coinsSpentOnSlayers.getInt("spider") ;
                }

                // Adds all of the retrieved information into a new Spider array index.
                Spider spiderProfileStats = new Spider(nameOfProfile,
                        spiderSlayerLevel, currentSpiderExp, neededSpiderExp,
                        tierOneSpiderKills, tierTwoSpiderKills, tierThreeSpiderKills, tierFourSpiderKills, coinsSpentOnSpiders);

                // Adds the new array index into the custom adapter
                spiderElement.add(spiderProfileStats);
        ...
        // Return the list of Spider slayer Stats
        return spiderElement;
    }
